<template>
  <q-page class="row items-start justify-between" style="max-height: 100%">
    ...
    <div class="col-7 justify-center" v-if="feuille_id" >
      <div class="q-pa-md q-gutter-sm row justify-between items-center">
        <q-btn @click="pmonth" align="left" class="btn-fixed-width" color="primary" label="Previous month" :disable="selectedIndex === 0"></q-btn>
        <small caption>{{ feuille_date }}</small>
        <q-btn @click="nmonth" align="right" class="btn-fixed-width" color="primary" label="Next month" :disable="selectedIndex === datas.length-1"></q-btn>
      </div>
